...
3. [**Functional Requirements (FR-1 to FR-17)**](../REQUIREMENTS.md#3-comprehensive-functional-requirements-fr)
   - **FR-1**: FastMCP 2.0 Dual Transport Architecture (SSE & Streamable HTTP).
   - **FR-2**: FastMCP 14 Agent Tools Contract (`search_code`, `search_docs`, `find_symbol`, `trace_path`, `find_routes`, `find_api_callers`, `get_file_outline`, `list_repositories`, `sync_repository`, `index_status`, `get_architecture`, `manage_adr`, `manage_local_file`, `what_is_ingested`).
   ...
   - **FR-7**: Contextual Markdown & Fallback Chunking.
   - **FR-8**: Pluggable Multi-Backend Vector Retrieval (pgvector, Qdrant & ChromaDB).
   - **FR-9**: Codebase & Dependency Topology Graph Engine.
